...
$MaxID = mysqli_query($mcon, "SELECT MAX(date) FROM `data11`");
$MaxID = mysqli_fetch_array($MaxID, MYSQL_BOTH);
$MaxID = $MaxID[0];
$MaxIT = mysqli_query($mcon, "SELECT MAX(time) FROM `data11`");
...
$MaxIT1 = $MaxIT1[0];
    ...
    while ($MaxID != $MaxID1 && $MaxIT != $MaxIT1)
{
   $result = mysqli_query($mcon,"SELECT * FROM data11 WHERE date=$MaxID1 AND time=$MaxIT1 ");
